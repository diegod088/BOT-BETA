...
def parse_telegram_link(url: str) -> tuple[str, int] | None:
    """Extract channel identifier and message ID from Telegram link"""
    # Private with invite hash
    match = re.search(r't\.me/\+([^/]+)/(\d+)', url)
    if match:
        return f"+{match.group(1)}", int(match.group(2))
    
    # Private with numeric ID
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    welcome_message += "\n\n━━━━━━━━━━━━━━━━━━━━\n"
    welcome_message += "🔗 Envía un enlace de Telegram para comenzar."
    
    # Add buttons: Premium plans and channel
    keyboard = [
        ...
        if not message.media and message.text:
            ...
            if inner_links:
                ...
                if inner_parsed:
                    ...
                    message = await telethon_client.get_messages(inner_entity, ids=inner_message_id)
        
        if not message or not message.media:
            await update.message.reply_text(
                "❌ *Error*\n\n"
                "No se encontró contenido multimedia.\n"
                "Verifica el enlace e intenta nuevamente.",
                parse_mode='Markdown'
            )
            return
